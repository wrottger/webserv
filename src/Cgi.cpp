...
Cgi::Cgi(const std::string &bodyBuffer, const int &epollFd, const HttpHeader &requestObject) : _isFinished(false), _errorCode(0), _sockets{-1, -1} {
...
void Cgi::executeCgi(const std::string &bodyBuffer, const int &epollFd, const HttpHeader &requestObject) {
	...
	try {
		...
		eventData->fd = _sockets[0];
		eventData->eventType = CLIENT;
		eventData->objectPointer = NULL;
		// TODO: Set objectPointer to Client object
		// TODO: Add to EventHandler EventData list
		epoll_event ev;
		ev.events = EPOLLIN | EPOLLET;
		ev.data.ptr = eventData;
		if (epoll_ctl(epollFd, EPOLL_CTL_ADD, _sockets[0], &ev) < 0) {
	...
	if (pid < 0) {
		...
		executeChild(bodyBuffer, requestObject);


}

int Cgi::excecuteChild(const std::string &bodyBuffer, const HttpHeader &requestObject) {
